...
address memory_allocation (str judul, str url){
	address temp;
...
void insert_first(List *L, str judul, str url){
	address temp = memory_allocation(judul, url);
...
void insert_last(List *L, str judul, str url){
	address temp, last;
	
	if(is_list_empty(*L)){
		insert_first(L, judul, url);
	}else{
		temp = memory_allocation(judul, url);
