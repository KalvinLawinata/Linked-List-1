...
address memory_allocation (str judul, str sutradara){
	address temp;
	
	temp = malloc(sizeof(tNode));
	
	strcpy(temp->judul, judul);
	strcpy(temp->sutradara, sutradara);
	temp->next = NULL;
	
	return temp;
}

void insert_first(List *L, str judul, str sutradara){
	address temp = memory_allocation(judul, sutradara);
...
void insert_last(List *L, str judul, str sutradara){
	address temp, last;
	
	if(is_list_empty(*L)){
		insert_first(L, judul, sutradara);
	}else{
		temp = memory_allocation(judul, sutradara);
...
void print_list(List L){
	...
	if(is_list_empty(L)){
		...
		while(temp!=NULL){
			printf("\n-------------------------");
			printf("\nJudul film : %s", temp->judul);
			printf("\nsutradara : %s", temp->sutradara);
			printf("\n-------------------------\n");
